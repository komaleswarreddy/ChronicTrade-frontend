...
export default function ComplianceEvaluationPanel({ simulationId }) {
  ...
  const fetchComplianceEvaluation = async () => {
    if (!isAuthenticated() || !simulationId) return
    ...
    try {
      ...
      const response = await api.get(`/api/compliance/${simulationId}/evaluation`)
  ...
  return (
    <div className="p-6">
      ...
      {/* RAG Citations */}
      {evaluation.rag_citations && evaluation.rag_citations.length > 0 && (
        <div className="mt-6">
          <CitationDisplay citations={evaluation.rag_citations} />
        </div>
      )}
    </div>
  )
