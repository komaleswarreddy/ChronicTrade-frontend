...
export default function ExecutionStepsPanel({ simulationId }) {
  ...
  const handleExecuteStep = async () => {
    if (!isAuthenticated() || !simulationId || executing) return
  ...
  const handleRetryFailedStep = async (stepId) => {
    if (!isAuthenticated() || !stepId || resetting === stepId) return
