...
export default function AdvisorCard({ proposal, onAction }) {
  ...
  const fetchNarrative = async () => {
    if (!isAuthenticated() || !proposal?.proposal_id) return
          ...
          onClick={async () => {
            if (!expanded && !detailedProposal && !fetchingDetail) {
              ...
              try {
                ...
                // api.js interceptor will automatically add the JWT token
                const detailRes = await api.get(`/api/agent/proposals/${proposal.proposal_id}`)
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {showWhatChanged && (
                  <div className="mt-4">
                    <WhatChangedPanel proposalId={proposal.proposal_id} />
                  </div>
                )}
                ...
                {(() => {
                  const displayProposal = detailedProposal || proposal
                  if (displayProposal.rag_citations && displayProposal.rag_citations.length > 0) {
                    return (
                      <div className="mt-4 pt-4 border-t border-gray-200">
                        <h5 className="text-sm font-semibold text-gray-900 mb-2">Knowledge Sources</h5>
                        <CitationDisplay citations={displayProposal.rag_citations} />
                      </div>
                    )
                  }
                  return null
