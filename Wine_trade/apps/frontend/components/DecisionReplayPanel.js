...
export default function DecisionReplayPanel({ simulationId }) {
  ...
  const fetchDecisionData = async () => {
    ...
    try {
      ...
      const [lineageRes, evalRes] = await Promise.all([
        api.get(`/api/audit/decision-lineage?simulation_id=${simulationId}`).catch(() => ({ data: { lineage: [] } })),
        api.get(`/api/audit/policy-evaluations?simulation_id=${simulationId}`).catch(() => ({ data: { evaluations: [] } }))
      ])
