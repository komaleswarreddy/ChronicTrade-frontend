...
export default function WhatChangedPanel({ proposalId }) {
  ...
  useEffect(() => {
    if (proposalId && isAuthenticated()) {
      fetchWhatChanged()
    }
  }, [proposalId, isAuthenticated])

  const fetchWhatChanged = async () => {
    if (!isAuthenticated() || !proposalId) return
    ...
    try {
      ...
      const [diffRes, narrativeRes] = await Promise.all([
        api.get(`/api/explainability/proposal-diff/${proposalId}`).catch(() => ({ data: { has_previous: false } })),
        api.get(`/api/explainability/narrative/${proposalId}`).catch(() => ({ data: { narrative: null } }))
      ])
