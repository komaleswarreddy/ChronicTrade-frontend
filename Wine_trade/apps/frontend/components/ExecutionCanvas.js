...
export default function ExecutionCanvas({ 
  ...
  useEffect(() => {
    if (!simulationId || !isAuthenticated()) return
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [stepsRes, complianceRes, gatesRes, logisticsRes, counterfactualRes] = await Promise.allSettled([
          api.get(`/api/executions/${simulationId}/steps`),
          api.get(`/api/compliance/${simulationId}/evaluation`).catch(() => ({ data: null })),
          api.get(`/api/executions/${simulationId}/gates`).catch(() => ({ data: null })),
          api.get(`/api/logistics/${simulationId}/timeline`).catch(() => ({ data: null })),
          api.get(`/api/counterfactual/${simulationId}`).catch(() => ({ data: null })),
        ])
