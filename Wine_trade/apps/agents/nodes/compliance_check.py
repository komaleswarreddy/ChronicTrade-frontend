...
Uses RAG to retrieve relevant compliance rules and risk policies.
"""
...
async def compliance_check_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Query RAG for relevant compliance rules and risk policies
        rag_citations = []
        rag_context = ""
        try:
            client = BackendAPIClient()
            # Build query from recommendation context
            query = f"compliance rules for {action} action with confidence {confidence:.2f} and expected ROI {expected_roi:.2f}"
            rag_response = await client.query_rag(
                query=query,
                source_types=["compliance_rule", "risk_policy"],
                top_k=3,
                min_confidence=0.5
            )
            rag_citations = rag_response.get("citations", [])
            # Build context from citations
            if rag_citations:
                rag_context = "\n".join([
                    f"- {cite.get('title', '')}: {cite.get('content', '')[:200]}..."
                    for cite in rag_citations[:3]
                ])
            await client.close()
        except Exception as e:
            logger.warning(f"RAG query failed (non-critical): {e}")
        ...
        # Include RAG context in reason if available
        if rag_context:
            compliance_reason += f"\n\nRelevant policies: {rag_context}"
        ...
        return {
            ...
            "rag_citations": rag_citations,  # Store citations in state
        }
