...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Portfolio Trend - CRITICAL FIX: Always update even if empty to trigger re-render
      if (trendRes.status === 'fulfilled') {
        const trendDataArray = trendRes.value.data || []
        console.log('[Dashboard] Trend data fetched:', { count: trendDataArray.length, latest: trendDataArray[trendDataArray.length - 1] })
        setTrendData(trendDataArray)
      } else {
        console.error('Failed to fetch portfolio trend:', trendRes.reason?.response?.status || trendRes.reason?.message, trendRes.reason)
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (trendRes.status === 'fulfilled') {
          const trendDataArray = trendRes.value.data || []
          console.log('[Dashboard] Auto-refresh trend data:', { count: trendDataArray.length })
          setTrendData(trendDataArray)
        } else {
          console.warn('[Dashboard] Auto-refresh trend failed:', trendRes.reason?.response?.status || trendRes.reason?.message)
        }
      } catch (err) {
                ...
                <button
                  onClick={async () => {
                    try {
                      const token = await getToken()
                      if (!token) {
                        console.error('No token available for trend refresh')
                        return
                      }
                      const authConfig = {
                        headers: {
                      ...
                      // CRITICAL FIX: Force fresh fetch with timestamp and cache-busting
                      const timestamp = new Date().getTime()
            ...
            <div className="mb-8">
              <SimulationHistoryTable 
                ...
                onSimulationUpdate={async () => {
                  // CRITICAL FIX: Refresh all data after simulation execution/update
                  ...
                  // CRITICAL: Use cache-busting timestamps to force fresh data
                  const timestamp = new Date().getTime()
                  const random = Math.random()
                  ...
                  if (holdingsRes.status === 'fulfilled') {
                    const holdingsData = holdingsRes.value.data || []
                    console.log('[Dashboard] Holdings refreshed:', { count: holdingsData.length })
                    setHoldings(holdingsData)
                  } else {
                    console.error('[Dashboard] Failed to refresh holdings:', holdingsRes.reason)
                  }
                  
                  if (summaryRes.status === 'fulfilled') {
                    console.log('[Dashboard] Portfolio summary refreshed')
                    setPortfolioSummary(summaryRes.value.data)
                  } else {
                    console.error('[Dashboard] Failed to refresh portfolio summary:', summaryRes.reason)
                  }
                  
                  if (trendRes.status === 'fulfilled') {
                    const trendDataArray = trendRes.value.data || []
                    console.log('[Dashboard] Trend data refreshed:', { count: trendDataArray.length })
                    setTrendData(trendDataArray)
                  } else {
                    console.error('[Dashboard] Failed to refresh trend:', trendRes.reason)
                  }
                  
                  console.log('[Dashboard] ✅ All data refreshed after simulation execution')
                }}
              />
            </div>
