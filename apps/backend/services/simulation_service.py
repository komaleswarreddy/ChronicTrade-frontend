...
def execute_simulation(user_id: str, simulation_id: str, conn=None) -> Dict:
    ...
    try:
        ...
        # CRITICAL FIX: Create holdings after successful execution
        # This ensures holdings appear in the user's portfolio after execution
        if simulation['action'] == 'BUY':
            try:
                from services.holdings_service import create_holding
                
                asset_id = simulation['asset_id']
                quantity = int(simulation.get('quantity') or 1)
                buy_region = simulation.get('buy_region') or simulation.get('region')
                
                logger.info(f"🔄 Creating holding for executed simulation {simulation_id}: BUY {quantity} of {asset_id}")
                
                # Get asset price for the holding
                asset_price = None
                try:
                    cursor.execute("""
                    ...
                    if asset_data:
                        asset_price = float(asset_data.get('base_price') or 0.0)
                        logger.info(f"📊 Asset price for {asset_id}: ₹{asset_price:,.2f}")
                    else:
                        logger.warning(f"⚠️ Asset {asset_id} not found in assets table")
                except Exception as e:
                    logger.error(f"❌ Failed to get asset price: {e}", exc_info=True)
                
                # Create holding with source as ARBITRAGE_SIMULATION (simulation-based execution)
                # Pass None for buy_price to use current market price if asset_price is not available
                holding = create_holding(
                    conn=conn,
                    user_id=user_id,
                    asset_id=asset_id,
                    quantity=quantity,
                    buy_price=asset_price if asset_price and asset_price > 0 else None,
                    source="ARBITRAGE_SIMULATION"
                )
                logger.info(f"✅ SUCCESS: Created holding {holding.get('id')} for executed simulation {simulation_id} (BUY {quantity} of {asset_id})")
            except ImportError as e:
                logger.error(f"❌ CRITICAL: holdings_service not available: {e}", exc_info=True)
            except Exception as e:
                logger.error(f"❌ CRITICAL: Failed to create holding after execution: {e}", exc_info=True)
                # Re-raise the exception to fail execution if holding creation fails
                # This ensures we know when holdings aren't being created
                raise ValueError(f"Failed to create holding after execution: {str(e)}")
        elif simulation['action'] == 'SELL':
            # For SELL actions, we need to find and update existing holdings
            try:
                from services.holdings_service import sell_holding
                
                asset_id = simulation['asset_id']
                quantity = int(simulation.get('quantity') or 1)
                
                # Find open holdings for this asset
                cursor.execute("""
                    SELECT id, quantity FROM holdings
                    WHERE user_id = %s AND asset_id = %s AND status = 'OPEN'
                    ORDER BY opened_at ASC
                """, (user_id, asset_id))
                
                holdings_to_sell = cursor.fetchall()
                if holdings_to_sell:
                    # Sell from holdings (partial or full)
                    total_available = sum(h['quantity'] for h in holdings_to_sell)
                        ...
                        for holding in holdings_to_sell:
                            ...
                            try:
                                sell_holding(
                                    ...
                                    quantity=sell_qty,
                                    sell_price=None  # Use current market price
                                )
                                remaining -= sell_qty
                                logger.info(f"✅ Sold {sell_qty} from holding {holding['id']} for executed simulation {simulation_id}")
                            except Exception as e:
                                logger.error(f"Failed to sell holding {holding['id']}: {e}", exc_info=True)
                    else:
                        logger.warning(f"Insufficient holdings to sell {quantity} of {asset_id} (only {total_available} available)")
                else:
                    logger.warning(f"No open holdings found to sell for asset {asset_id}")
            except ImportError:
                logger.warning("holdings_service not available, skipping holding sale")
            except Exception as e:
                logger.error(f"❌ Failed to sell holding after execution: {e}", exc_info=True)
                # Don't fail execution if holding sale fails
        ...
        # CRITICAL FIX: Create portfolio snapshot after execution to update trend
        try:
            from services.snapshot_service import create_portfolio_snapshot
            create_portfolio_snapshot(conn, user_id, force_update=True)
            logger.info(f"✅ Updated portfolio snapshot after execution of simulation {simulation_id}")
        except Exception as e:
            logger.warning(f"Failed to update portfolio snapshot after execution: {e}", exc_info=True)
