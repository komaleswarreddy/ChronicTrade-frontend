...
def get_portfolio_trend(conn, user_id: str, days: int = 30, ensure_today: bool = True) -> List[Dict]:
    ...
    """
    # Ensure today's snapshot exists for real-time data
    if ensure_today:
    ...
    # Always ensure today is included with current value (most up-to-date)
    if not today_in_data or (trend_data and trend_data[-1]["date"] != today_formatted):
        # Get current portfolio value (most accurate)
        summary = calculate_portfolio_summary(conn, user_id)
